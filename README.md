...
- [x] [日常钓鱼](/json/日常钓鱼.json)
  > 包含摩尔拉雅、浆果丛林、阳光沙滩钓鱼点，暂不包含家园钓鱼点，如有需要请使用下方的 `辅助钓鱼`。  
  > 运行前需要：
  > > - **初级鱼饵 > 450** *(因为会有少量丢饵情况)*

- [x] [日常打工人整合](/json/日常打工人整合.json)
  > 在 `日常整合` 的基础上加入了 `日常夜间资源采集` 和 `浆果丛林采集`
  > 推荐打工人使用，只需要晚上睡前启动即可
  > 运行前需要： 
  > > - 换服装（需要穿齐套装）为 **套装：向导服**
  > > - 换载具（不需要骑乘，装扮里设置即可）为 **载具：驴牌滑板车**
...
4. 手动设置摩尔庄园
   - **刘海屏：关**
5. 下载项目 [json](/json) 文件夹中对应的 json 文件
6. 导入 json 文件为宏脚本
